...
start_time = "09:44:00"
end_time = "14:41:00"
date_list_filename = "Backtest_Dates_Tue.csv"
output_filename = "C:/Users/vidya/Desktop/Options_RK/Jatin/OptionsCode/Tue.csv"
...
day_profit = {}
date_list = ["03-11-2021"]
             # , "02-11-2021", "03-11-2021", "08-11-2021", "09-11-2021", "10-11-2021"]
    ...
    for index_,row in time_temp[time_temp.index >= start_time].iterrows():
        ...
        if((position_active_call == True) & (position_active_put == True) & (check_combined_stoploss == True)):
            ...
            if(curr_combined_prem - combined_premium >= max_loss):
                ...
                buy_call_price = time_temp[call_file_name[:-8]+"_SellPrice"][index_]
                position_active_call = False
                # if(position_active_put == True):
                buy_put_price = time_temp[put_file_name[:-8]+"_SellPrice"][index_]                
        ...
        if(position_active_call == True):
            ...
            if(time_temp[call_file_name[:-8]+"_SellPrice"][index_] >= (1+stoploss_percent)*sell_price_call):
                ...
                print("Stop loss hit for call bought call option buying at : ",index_, " for : ",buy_call_price , " making profit of : ", (sell_price_call-buy_call_price))
                index_c  = index_
                idx_c = np.searchsorted(time_temp.index, index_)
                ...
                buy_call_price = time_temp[call_file_name[:-8]+"_SellPrice"][index_]
                position_active_call = False
                print("combine Stop loss hit for call option buying at : ",index_, " for : ",buy_call_price, " making profit of : ", (sell_price_call-buy_call_price))
                index_c  = index_
                idx_c = np.searchsorted(time_temp.index, index_)
                ...
                buy_call_price = time_temp[call_file_name[:-8]+"_SellPrice"][index_]
                position_active_call = False
                print("Time Close hit for call option buying at : ",index_, " for : ",buy_call_price, " making profit of : ", (sell_price_call-buy_call_price))
                index_c  = index_
                idx_c = np.searchsorted(time_temp.index, index_)
        ...
        if(position_active_put == True):
            ...
            if(time_temp[put_file_name[:-8]+"_SellPrice"][index_] >= (1+stoploss_percent)*sell_price_put):
                ...
                print("Stop loss hit for put bought put option buying at : ",index_, " for : ",buy_put_price , " making profit of : ", (sell_price_put-buy_put_price) )            
                index_p  = index_
                idx_p = np.searchsorted(time_temp.index, index_)
                ...
                buy_put_price = time_temp[put_file_name[:-8]+"_SellPrice"][index_]
                position_active_put = False
                print("combine Stop loss hit for put option buying at : ",index_, " for : ",buy_put_price , " making profit of : ", (sell_price_put-buy_put_price))
                index_p  = index_
                idx_p = np.searchsorted(time_temp.index, index_)
                ...
                buy_put_price = time_temp[put_file_name[:-8]+"_SellPrice"][index_]
                position_active_put = False
                print("Time close hit for put option buying at : ",index_, " for : ",buy_put_price , " making profit of : ", (sell_price_put-buy_put_price))
                index_p  = index_
                idx_p = np.searchsorted(time_temp.index, index_)
...
# day_profit_df.to_csv(output_filename)

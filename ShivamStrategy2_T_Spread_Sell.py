...
date_list = ["01-02-2021","02-02-2021","03-02-2021","04-02-2021","05-02-2021","08-02-2021","09-02-2021","10-02-2021","11-02-2021","12-02-2021","15-02-2021","16-02-2021","17-02-2021","18-02-2021"]
# date_list = ["17-02-2021"]
put_qtyL = 0
call_qtyL = 0
future_price =0
ref_future_price = 0
...
for date in date_list:
    ...
    if(call_qtyL!=0):
        ...
        # otm_strikeL = round(upper_band1 / roundoff_factor) * roundoff_factor
        call_file_nameL=instrument.upper()+expiry_date[:2]+(expiry_month_name[slice(3)]).upper()+expiry_date[8:]+str(otm_strikeL)+str("CE.NFO.csv")
        df_call_option = pd.read_csv(zf1.open(folder_name_ + call_file_nameL))
        time_tempCL = get_data(df_put_option,df_call_option)
        # otm_strikeS = round(upper_band2 / roundoff_factor) * roundoff_factor
        call_file_nameS=instrument.upper()+expiry_date[:2]+(expiry_month_name[slice(3)]).upper()+expiry_date[8:]+str(otm_strikeS)+str("CE.NFO.csv")
        df_call_option = pd.read_csv(zf1.open(folder_name_ + call_file_nameS))
        time_tempCS = get_data(df_put_option,df_call_option)
        ...
        # otm_strikeL = round(lower_band1 / roundoff_factor) * roundoff_factor
        put_file_nameL=instrument.upper()+expiry_date[:2]+(expiry_month_name[slice(3)]).upper()+expiry_date[8:]+str(otm_strikeL)+str("PE.NFO.csv")
        df_put_option = pd.read_csv(zf1.open(folder_name_ + put_file_nameL))
        time_tempPL = get_data(df_put_option,df_call_option)
        # otm_strikeS = round(lower_band2 / roundoff_factor) * roundoff_factor
        put_file_nameS=instrument.upper()+expiry_date[:2]+(expiry_month_name[slice(3)]).upper()+expiry_date[8:]+str(otm_strikeS)+str("PE.NFO.csv")
        df_put_option = pd.read_csv(zf1.open(folder_name_ + put_file_nameS))
        time_tempPS = get_data(df_put_option,df_call_option)
    ...
    for index_ in tqdm(df_weekly_futures.Time[(df_weekly_futures.Time >= start_time) & (df_weekly_futures.Time <= end_time)]):
        ...
        if((put_qtyL == 0) & (call_qtyL == 0)):
            ...
            if(put_buy_signal == 1):
                df_call_option = pd.DataFrame(columns = ['Ticker', 'Date', 'Time', 'LTP', 'BuyPrice', 'BuyQty', 'SellPrice','SellQty', 'LTQ', 'OpenInterest'])
                otm_strikeS = round(lower_band1 / roundoff_factor) * roundoff_factor
                put_file_nameS=instrument.upper()+expiry_date[:2]+(expiry_month_name[slice(3)]).upper()+expiry_date[8:]+str(otm_strikeS)+str("PE.NFO.csv")
                df_put_option = pd.read_csv(zf1.open(folder_name_ + put_file_nameS))
                # time_tempPL = get_data(df_put_option,df_call_option)
                # buy_price_putL = time_tempPL["Put_SellPrice"].loc[start_time]
                time_tempPS = get_data(df_put_option,df_call_option)
                sell_price_putS = time_tempPS["Put_BuyPrice"].loc[start_time]
                put_qtyS = -1
                if spread_variant == 1:
                    otm_strikeL = round(lower_band2 / roundoff_factor) * roundoff_factor
                    put_file_nameL=instrument.upper()+expiry_date[:2]+(expiry_month_name[slice(3)]).upper()+expiry_date[8:]+str(otm_strikeL)+str("PE.NFO.csv")
                    df_put_option = pd.read_csv(zf1.open(folder_name_ + put_file_nameL))
                    # time_tempPS = get_data(df_put_option,df_call_option)
                    # sell_price_putS = time_tempPS["Put_BuyPrice"].loc[start_time]
                    ...
                    buy_price_putL = np.nan
                    put_qtyL = np.nan
                     
                total_transactions+=1
                trade_type = 'Put_Spread_BUY'
                
                transactions_book.append([date,date_name,index_,future_price,future_price,vixdf.loc[date].values[0],upper_band2,upper_band1,lower_band1,lower_band2,put_file_nameL[:-8],buy_price_putL,put_qtyL,put_file_nameS[:-8],sell_price_putS,put_qtyS,0,0,0,0,0,trade_type])
                put_buy_signal = 0 
            ...
            if(call_buy_signal == 1):
                df_put_option = pd.DataFrame(columns = ['Ticker', 'Date', 'Time', 'LTP', 'BuyPrice', 'BuyQty', 'SellPrice','SellQty', 'LTQ', 'OpenInterest'])
                otm_strikeS = round(upper_band1 / roundoff_factor) * roundoff_factor
                call_file_nameS=instrument.upper()+expiry_date[:2]+(expiry_month_name[slice(3)]).upper()+expiry_date[8:]+str(otm_strikeS)+str("CE.NFO.csv")
                df_call_option = pd.read_csv(zf1.open(folder_name_ + call_file_nameS))
                time_tempCS = get_data(df_put_option,df_call_option)
                sell_price_callS = time_tempCS["Call_BuyPrice"].loc[start_time]
                
                call_qtyS = -1
                if spread_variant == 1:
                    otm_strikeL = round(upper_band2 / roundoff_factor) * roundoff_factor
                    call_file_nameL=instrument.upper()+expiry_date[:2]+(expiry_month_name[slice(3)]).upper()+expiry_date[8:]+str(otm_strikeL)+str("CE.NFO.csv")
                    df_call_option = pd.read_csv(zf1.open(folder_name_ + call_file_nameL))
                    time_tempCL = get_data(df_put_option,df_call_option)
                    buy_price_callL = time_tempCL["Call_SellPrice"].loc[start_time]
                    call_qtyL = 1
                else:
                    buy_price_callL = np.nan
                    call_qtyL = np.nan
                
                trade_type = 'Call_Spread_BUY'
                
                transactions_book.append([date,date_name,index_,future_price,future_price,vixdf.loc[date].values[0],upper_band2,upper_band1,lower_band1,lower_band2,call_file_nameL[:-8],buy_price_callL,call_qtyL,call_file_nameS[:-8],sell_price_callS,call_qtyS,0,0,0,0,0,trade_type])
        ...
        if(index_ <= last_trade_open_time):
            ...
            if (current_future_price > upper_band1):
                ...
                if (put_qtyL !=0) : 
                    buy_price_putS = time_tempPS["Put_SellPrice"].loc[index_]
                    put_qtyS = -1*put_qtyS
                    if spread_variant == 1:
                        sell_price_putL = time_tempPL["Put_BuyPrice"].loc[index_]
                        put_qtyL = -1*put_qtyL
                    else:
                        sell_price_putL = np.nan
                        put_qtyL = np.nan
                    
                    # put_qtyS = -1*put_qtyS
                    total_transactions+=1
                    profit = (sell_price_putL + sell_price_putS) - (buy_price_putL + buy_price_putS)
                    ...
                    transactions_book.append([date,date_name,index_,current_future_price,ref_future_price,vixdf.loc[date].values[0],upper_band2,upper_band1,lower_band1,lower_band2,put_file_nameL[:-8],0,0,put_file_nameS[:-8],0,0,sell_price_putL,put_qtyL,buy_price_putS,put_qtyS,profit,trade_type])
                    put_qtyL = 0
                    put_qtyS = 0
                    
                if (call_qtyL !=0):
                    buy_price_callS = time_tempCS["Call_SellPrice"].loc[index_]
                    call_qtyS = -1*call_qtyS
                    if spread_variant == 1:
                        sell_price_callL = time_tempCL["Call_BuyPrice"].loc[index_]
                        call_qtyL = -1*call_qtyL
                    else:
                        sell_price_callL = np.nan
                        call_qtyL = np.nan
                        
                    total_transactions+=1
                    profit = (sell_price_callL + sell_price_callS) - (buy_price_callL + buy_price_callS)
                    trade_type = 'Call_Spread_SELLRU'
                    ...
                    transactions_book.append([date,date_name,index_,current_future_price,ref_future_price,vixdf.loc[date].values[0],upper_band2,upper_band1,lower_band1,lower_band2,call_file_nameL[:-8],0,0,call_file_nameS[:-8],0,0,sell_price_callL,call_qtyL,buy_price_callS,call_qtyS,profit,trade_type])
                    call_qtyL = 0
                    call_qtyS = 0
                ...
                df_call_option = pd.read_csv(zf1.open(folder_name_ + call_file_nameS))
                time_tempCS = get_data(df_put_option,df_call_option)
                sell_price_callS = time_tempCS["Call_BuyPrice"].loc[index_]
                call_qtyS = -1
                if spread_variant == 1:
                    otm_strikeL = round(upper_band2 / roundoff_factor) * roundoff_factor
                    call_file_nameL=instrument.upper()+expiry_date[:2]+(expiry_month_name[slice(3)]).upper()+expiry_date[8:]+str(otm_strikeL)+str("CE.NFO.csv")
                    df_call_option = pd.read_csv(zf1.open(folder_name_ + call_file_nameL))
                    time_tempCL = get_data(df_put_option,df_call_option)
                    buy_price_callL = time_tempCL["Call_SellPrice"].loc[index_]
                    call_qtyL = 1
                else:
                    buy_price_callL = np.nan
                    call_qtyL = np.nan
                
                trade_type = 'Call_Spread_BUYRUA'
                ...
                transactions_book.append([date,date_name,index_,current_future_price,current_future_price,vixdf.loc[date].values[0],upper_band2,upper_band1,lower_band1,lower_band2,call_file_nameL[:-8],buy_price_callL,call_qtyL,call_file_nameS[:-8],sell_price_callS,call_qtyS,0,0,0,0,0,trade_type])
                total_transactions+=1
            ...
            # elif(time_temp['Futures_Price'].loc[index_] < lower_band):
                ...
                df_put_option = pd.read_csv(zf1.open(folder_name_ + put_file_nameS))
                time_tempPS = get_data(df_put_option,df_call_option)
                sell_price_putS = time_tempPS["Put_BuyPrice"].loc[index_]
                put_qtyS = -1
                if spread_variant == 1:
                    otm_strikeL = round(lower_band2 / roundoff_factor) * roundoff_factor
                    put_file_nameL=instrument.upper()+expiry_date[:2]+(expiry_month_name[slice(3)]).upper()+expiry_date[8:]+str(otm_strikeL)+str("PE.NFO.csv")
                    df_put_option = pd.read_csv(zf1.open(folder_name_ + put_file_nameL))
                    time_tempPL = get_data(df_put_option,df_call_option)
                    buy_price_putL = time_tempPL["Put_SellPrice"].loc[index_]
                    put_qtyL = 1
                else:
                    buy_price_putL = np.nan
                    put_qtyL = np.nan
                trade_type = 'Put_Spread_BUYRLA'
 
                # print(date,index_,"Rollover done Call sold at :", sell_price_call,"new strike price is :", call_file_name,"Futures at :", future_price)
                transactions_book.append([date,date_name,index_,current_future_price,current_future_price,vixdf.loc[date].values[0],upper_band2,upper_band1,lower_band1,lower_band2,put_file_nameL[:-8],buy_price_putL,put_qtyL,put_file_nameS[:-8],sell_price_putS,put_qtyS,0,0,0,0,0,trade_type])
                total_transactions+=1
                ref_future_price = current_future_price
            ...
            if(put_qtyL!=0):
                ...
                otm_strikeS = round(lower_band1 / roundoff_factor) * roundoff_factor
                put_file_nameS=instrument.upper()+new_expiry[:2]+(expiry_month_name[slice(3)]).upper()+new_expiry[8:]+str(otm_strikeS)+str("PE.NFO.csv")
                df_put_option = pd.read_csv(zf1.open(folder_name_ + put_file_nameS))
                time_tempPS = get_data(df_put_option,df_call_option)
                sell_price_putS = time_tempPS["Put_BuyPrice"].loc[index_]
                put_qtyS = -1
                if spread_variant == 1:
                    otm_strikeL = round(lower_band2 / roundoff_factor) * roundoff_factor
                    df_call_option = pd.DataFrame(columns = ['Ticker', 'Date', 'Time', 'LTP', 'BuyPrice', 'BuyQty', 'SellPrice','SellQty', 'LTQ', 'OpenInterest'])
                    put_file_nameL=instrument.upper()+new_expiry[:2]+(expiry_month_name[slice(3)]).upper()+new_expiry[8:]+str(otm_strikeL)+str("PE.NFO.csv")
                    df_put_option = pd.read_csv(zf1.open(folder_name_ + put_file_nameL))
                    time_tempPL = get_data(df_put_option,df_call_option)
                    buy_price_putL = time_tempPL["Put_SellPrice"].loc[index_]
                    put_qtyL = 1
                ...
                transactions_book.append([date,date_name,index_,current_future_price,ref_future_price,vixdf.loc[date].values[0],upper_band2,upper_band1,lower_band1,lower_band2,put_file_nameL[:-8],buy_price_putL,put_qtyL,put_file_nameS[:-8],sell_price_putS,put_qtyS,0,0,0,0,0,trade_type])
                total_transactions+=1
            ...
            if(call_qtyL!=0):
                ...
                df_put_option = pd.DataFrame(columns = ['Ticker', 'Date', 'Time', 'LTP', 'BuyPrice', 'BuyQty', 'SellPrice','SellQty', 'LTQ', 'OpenInterest'])
                otm_strikeS = round(upper_band1 / roundoff_factor) * roundoff_factor
                call_file_nameS=instrument.upper()+new_expiry[:2]+(expiry_month_name[slice(3)]).upper()+new_expiry[8:]+str(otm_strikeS)+str("CE.NFO.csv")
                df_call_option = pd.read_csv(zf1.open(folder_name_ + call_file_nameS))
                time_tempCS = get_data(df_put_option,df_call_option)
                sell_price_callS = time_tempCS["Call_BuyPrice"].loc[index_]
                call_qtyS = -1
                if spread_variant == 1:
                    otm_strikeL = round(upper_band2 / roundoff_factor) * roundoff_factor
                    call_file_nameL=instrument.upper()+new_expiry[:2]+(expiry_month_name[slice(3)]).upper()+new_expiry[8:]+str(otm_strikeL)+str("CE.NFO.csv")
                    df_call_option = pd.read_csv(zf1.open(folder_name_ + call_file_nameL))
                    time_tempCL = get_data(df_put_option,df_call_option)
                    buy_price_callL = time_tempCL["Call_SellPrice"].loc[index_]
                    call_qtyL = 1
                    ...
                    buy_price_callL = np.nan
                    call_qtyL = np.nan
                trade_type = 'Call_Spread_BUYRT'
                # print(date,index_,"Rollover done Call sold  at :", sell_price_call,"new strike price is :", call_file_name,"Futures at :", future_price)
                transactions_book.append([date,date_name,index_,current_future_price,ref_future_price,vixdf.loc[date].values[0],upper_band2,upper_band1,lower_band1,lower_band2,call_file_nameL[:-8],buy_price_callL,call_qtyL,call_file_nameS[:-8],sell_price_callS,call_qtyS,0,0,0,0,0,trade_type])
                total_transactions+=1

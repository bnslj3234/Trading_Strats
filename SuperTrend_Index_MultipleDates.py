...
def EMA(dff, base, target, period, alpha=False):
    
    dff[target] = np.nan
    
    sma = dff[base].rolling(period, min_periods=period).mean()
    ...
    rest = dff[base][idx_end:]
    dff[target] = pd.concat([sma, rest]).ewm(alpha=1 / period, adjust=False).mean()

    # df1 = df.iloc[(period+1):]
    ...
    dff[target].fillna(0, inplace=True)
    
    # df.index = indexx
    return dff

def ATR(extra_data,df, period, ohlc=['open', 'high', 'low', 'close']):

    atr = 'ATR_' + str(period)
    
    # indexx = df.index[(period+1):]
    ...
    df.reset_index(drop = False, inplace = True)
    df1 = EMA(df, 'TR', atr, period, alpha=False)
    
    return df1

def SuperTrend(extra_data,dfy, period = 10, multiplier=3, ohlc=['open', 'high', 'low', 'close']):
    """
    ...
    """
    dfy = pd.concat([extra_data,dfy])
    df = ATR(extra_data,dfy, period, ohlc=ohlc)
    ...
    df1f = df.iloc[period+1:]
    df1f.set_index('index', inplace = True)
    
    return df1f


def make_candle_data(extra_data,df_fut, candle_freq_min):
...
extra_data = pd.DataFrame(index = ['15:00:00', '15:03:00', '15:06:00', '15:09:00', '15:12:00', '15:15:00',
       '15:18:00', '15:21:00', '15:24:00', '15:27:00', '15:30:00'], columns = ['open', 'high', 'low', 'close'])
for date in date_list:
    ...
    bars,bars_call_put_data = make_candle_data(extra_data,df_weekly_futures, candle_freq_min)
    extra_data = bars[['open', 'high', 'low', 'close']].iloc[-(supertrend_period+1):]

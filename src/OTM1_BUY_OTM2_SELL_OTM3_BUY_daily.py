...
transactionbook = [["Trade_Start_date", "Date_name","expiry_date", "start_time", "future_price", "Call_Ticker", "Put_Ticker",
                    "Start_ATM_IV", "call_sell_price", "put_sell_price","Hedge_Call_Ticker", "Hedge_Put_Ticker","Hedge_Call_Buy_Price","Hedge_Put_Buy_Price","OTM3_Call_Ticker", "OTM3_Put_Ticker","OTM3_Call_Buy_Price","OTM3_Put_Buy_Price", "stop_date", "stop_date_name", "end_time",
                    "future_price_end","End_ATM_IV", "call_buy_price", "put_buy_price","Hedge_Call_Sell_Price","Hedge_Put_Sell_Price","OTM3_Call_Sell_Price","OTM3_Put_Sell_Price","min_profit","max_profit","Till_Date_Min","Till_Date_Max","trade_type",]]
...
for date in date_list:
    ...
    call_sell_price = time_temp_otm2["Call_BuyPrice"].loc[start_time]
    put_sell_price = time_temp_otm2["Put_BuyPrice"].loc[start_time]
    ...
    call_sell_price_otm3 = time_temp_otm3["Call_BuyPrice"].loc[start_time]
    put_sell_price_otm3 = time_temp_otm3["Put_BuyPrice"].loc[start_time]
    ...
    for stop_date in between_days:
        ...
        time_temp = get_data(df_put_option_otm2, df_call_option_otm2)
        ...
        for index_,row in tqdm(time_temp[time_temp.index >= start_time].iterrows()):
            ...
            if call_position_active == True:
                ...
                call_buy_price_otm3 = time_temp_otm3["Call_SellPrice"].loc[index_]
                if buy_call_hedge == True:
                    ...
                    call_profit = 2*(call_sell_price - call_buy_price) + hedge_call_profit + (call_sell_price_otm3-call_buy_price_otm3)
                else:
            ...
            if put_position_active == True:
                ...
                put_buy_price_otm3 = time_temp_otm3["Put_SellPrice"].loc[index_]
                if buy_put_hedge == True:
                    hedge_put_sell_price = time_temp_hedge["Put_BuyPrice"].loc[index_]
                    hedge_put_profit = (hedge_put_sell_price - hedge_put_buy_price)
                    put_profit = 2*(put_sell_price - put_buy_price) + hedge_put_profit+(put_sell_price_otm3-put_buy_price_otm3)
                else:
            ...
            if((call_profit < -1*(individual_stoploss_fraction)*(call_sell_price)) & (call_position_active == True) & (check_individual_stoploss == True)):
                ...
                transactionbook.append([trade_start_date, date_name,expiry_date, start_time_t, future_price, otm2_call_file_name[:-10], otm2_put_file_name[:-10],start_atm_IV, call_sell_price, put_sell_price, hedge_call_file_name[:-10],hedge_put_file_name[:-10],hedge_call_buy_price,hedge_put_buy_price,otm3_call_file_name[:-10],otm3_put_file_name[:-10],call_sell_price_otm3,put_sell_price_otm3,trade_end_date, stop_date_name, index_,future_price_end, end_atm_IV,call_buy_price, put_buy_price,hedge_call_sell_price,hedge_put_sell_price,call_buy_price_otm3,put_buy_price_otm3,daily_max_profit,till_date_min_profit,till_date_min_profit,till_date_max_profit,trade_type])
            ...
            if((stop_date == expiry_date) & (index_ == end_time) & ((call_position_active == True) | (put_position_active == True))):
                ...
                trade_end_date = stop_date
                transactionbook.append([trade_start_date, date_name,expiry_date, start_time_t, future_price, otm2_call_file_name[:-10], otm2_put_file_name[:-10],start_atm_IV, call_sell_price, put_sell_price, hedge_call_file_name[:-10],hedge_put_file_name[:-10],hedge_call_buy_price,hedge_put_buy_price,otm3_call_file_name[:-10],otm3_put_file_name[:-10],call_sell_price_otm3,put_sell_price_otm3,trade_end_date, stop_date_name, index_,future_price_end, end_atm_IV,call_buy_price, put_buy_price,hedge_call_sell_price,hedge_put_sell_price,call_buy_price_otm3,put_buy_price_otm3,daily_min_profit,daily_max_profit,till_date_min_profit,till_date_max_profit,trade_type])
                break
...
transactionbook_df["Call_Profit"] = 2*(transactionbook_df['call_sell_price'] - transactionbook_df['call_buy_price'])
transactionbook_df["Put_Profit"] = 2*(transactionbook_df['put_sell_price'] - transactionbook_df['put_buy_price'])
...
transactionbook_df["Total_Profit"] = transactionbook_df["Call_Profit"] + transactionbook_df["Put_Profit"]+transactionbook_df["Call_Hedge_Profit"]+transactionbook_df["Put_Hedge_Profit"]+transactionbook_df["Call_OTM3_Profit"]+transactionbook_df["Put_OTM3_Profit"]
transactionbook_df.to_csv("../res/OTM1_BUY_OTM2_SELL_OTM3_BUY_daily_"+str(extra_offset)+".csv", index = False) 
